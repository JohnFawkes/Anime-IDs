...
Last generated at: August 18, 2026 09:04 AM UTC
...
2. [notseteve/AnimeAggregations](https://github.com/notseteve/AnimeAggregations) parses data from the AniDB site and maps the links there to TMDb IDs, IMDb IDs, and MyAnimeList IDs. ([Raw List](https://raw.githubusercontent.com/notseteve/AnimeAggregations/main/aggregate/AnimeToExternal.json))
3. The [AniList API](https://docs.anilist.co/) maps every MyAnimeList ID found above to its AniList ID.

AniList IDs are looked up by MyAnimeList ID, so an AniDB ID only gets an `anilist_id` if it first gets a `mal_id`. To stay inside the AniList rate limit only MyAnimeList IDs that aren't already mapped in `anime_ids.json` are looked up on each run.

## Make Changes

To change ID's mapped in the autogenerated `anime_ids.json` you can either update the [Anime-Lists/anime-lists](https://github.com/Anime-Lists/anime-lists/) List for TVDb IDs and IMDb IDs or add the MyAnimeList link to the anime's Resources on AniDB for MyAnimeList IDs and AniList IDs.
...
- `AniDB ID not found for MyAnimeList ID: #####`
  - **Issue:** Kometa received a MyAnimeList ID from a Builder and no AniDB ID is mapped to that MyAnimeList ID on [AnimeAggregations](https://raw.githubusercontent.com/notseteve/AnimeAggregations/main/aggregate/AnimeToExternal.json). 
  - **Solution:** [notseteve/AnimeAggregations](https://github.com/notseteve/AnimeAggregations) is an automated list built from the Resources listed on the AniDB page. You can try and add the MyAnimeList link to that anime's Resources on AniDB but I don't know when or even if that info will populate back to this list.

- `AniDB ID not found for AniList ID: #####`
  - **Issue:** AniList IDs are looked up from the MyAnimeList ID mapped to each AniDB ID, so either no MyAnimeList ID is mapped to the AniDB ID or the [AniList API](https://docs.anilist.co/) has no anime linked to that MyAnimeList ID. 
  - **Solution:** Add the MyAnimeList link to that anime's Resources on AniDB, and make sure the AniList entry has its MyAnimeList ID set. Neither list accepts Pull Requests, so I don't know when or even if that info will populate back to this list.
